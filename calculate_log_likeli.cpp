...
// [[Rcpp::export]]
double log_zinb(int x, double mu, double phi) {
  double lpmf_zinb;
  lpmf_zinb = R::dnbinom_mu(x, phi, mu, 1);
  return(lpmf_zinb);
}

// [[Rcpp::export]]
mat calculate_prob(umat data_x1, mat theta_fit, mat mu_c_fit, mat phi_c_fit, vec day_indx){
  int num_cells = data_x1.n_rows;
  int num_clust = theta_fit.n_rows;
  int num_prot  = mu_c_fit.n_rows;
  
  double theta_c_t, log_lik_i;
  double mu_c_p, phi_c_p, mu_i;
  int cell_indx, clust_i, prot_i;
  
  mat log_theta_mat = log(theta_fit + 1e-10);
  urowvec cell_i;
  arma::mat all_cells(num_cells, 1);
  ...
  for(cell_indx = 0; cell_indx < num_cells; cell_indx++){
    ...
    for(clust_i  = 0; clust_i < num_clust; clust_i++) {
      ...
      for(prot_i = 0; prot_i < num_prot; prot_i++){
        ...
        mu_i    = mu_c_p;
        log_lik_i = log_lik_i + log_zinb(cell_i(prot_i), mu_i, phi_c_p);
      }
      
      clust_lik(clust_i) = log_lik_i + theta_c_t;
      
      // all_cells(cell_indx, clust_i) = clust_lik();
    }
    
    all_cells(cell_indx) = log(sum(exp(clust_lik)));
...
// [[Rcpp::export]]
mat calculate_max_prob(umat data_x1, mat theta_fit, mat mu_c_fit, mat phi_c_fit, vec day_indx){
  int num_cells = data_x1.n_rows;
  int num_clust = theta_fit.n_rows;
  int num_prot  = mu_c_fit.n_rows;
  ...
  for(cell_indx = 0; cell_indx < num_cells; cell_indx++){
    ...
    // cout << clust_lik.index_max();
    all_cells(cell_indx) = clust_lik(clust_lik.index_max());
      
    // log(sum(exp(clust_lik)));
    // all_cells.row(cell_indx) = exp(all_cells.row(cell_indx) - all_cells.row(cell_indx).max());
    // all_cells.row(cell_indx) = all_cells.row(cell_indx) / sum(all_cells.row(cell_indx));
...
// [[Rcpp::export]]
mat predict_cluster_prob(umat data_x1, mat theta_fit, mat mu_c_fit, mat phi_c_fit, vec day_indx){
  int num_cells = data_x1.n_rows;
  int num_clust = theta_fit.n_rows;
  int num_prot  = mu_c_fit.n_rows;
  ...
  for(cell_indx = 0; cell_indx < num_cells; cell_indx++){
   ...
   all_cells.row(cell_indx) = exp(all_cells.row(cell_indx) - all_cells.row(cell_indx).max());
   if(isnan(sum(all_cells.row(cell_indx))) == 1) cout<<cell_indx<<" ";
   all_cells.row(cell_indx) = all_cells.row(cell_indx) / sum(all_cells.row(cell_indx));
   // cout<<isnan(all_cells.row(cell_indx))<<" ";
     //.index_max() + 1;
    //all_cells(cell_indx) = clust_lik.index_max() + 1;
    // cout << cell_indx;
 }
...
// [[Rcpp::export]]
vec predict_cluster(umat data_x1, mat theta_fit, mat mu_c_fit, mat phi_c_fit, vec day_indx){
  int num_cells = data_x1.n_rows;
  int num_clust = theta_fit.n_rows;
  int num_prot  = mu_c_fit.n_rows;
  double st_i, theta_c_t, log_lik_i;
  ...
  mat prob_matx = predict_cluster_prob(data_x1, theta_fit, mu_c_fit, phi_c_fit, day_indx);
